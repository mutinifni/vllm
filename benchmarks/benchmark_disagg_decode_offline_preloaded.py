...
def decode_benchmark(args):
    """
    Benchmark decode-only vLLM instance with PreloadedSharedStorageConnector.

    1. Read manifest
    2. Instantiate LLM with kv_transfer_config (PreloadedSharedStorageConnector, kv_both, local_storage)
    ...
    print(f"[Preloaded Decode] Using {len(prompts)} prompts from manifest.")

    sampling_params = SamplingParams(temperature=0, top_p=0.95, max_tokens=args.max_tokens)

    print("[Preloaded Decode] Creating LLM with PreloadedSharedStorageConnector...")
    llm = LLM(
        model=args.model,
        enforce_eager=True,
    ...
    for i, output in enumerate(outputs):
        ...
        print(f"[Preloaded Decode] Prompt {i}: latency={latencies[-1]:.3f}s, output={output.outputs[0].text[:40]!r}")

    print("\n==== Preloaded Decode Benchmark Results ====")
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark decode-only vLLM instance with PreloadedSharedStorageConnector.")
