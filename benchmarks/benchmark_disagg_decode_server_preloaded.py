...
def _strip_kv_cache_args(argv: list[str]) -> list[str]:
    """Return a copy of *argv* without the --kv-cache-dir and --manifest flags/values."""
    stripped: list[str] = []
    skip_next = False
    for tok in argv:
        if skip_next:
            skip_next = False
            continue
        if tok in ["--kv-cache-dir", "--manifest"]:
            skip_next = True  # Skip its value, too.
            continue
        if tok.startswith("--kv-cache-dir=") or tok.startswith("--manifest="):
            continue
        stripped.append(tok)
    return stripped
...
def main() -> None:
    ...
    parser = EngineArgs.add_cli_args(
        FlexibleArgumentParser(
            description=(
                "Start a decode-only vLLM OpenAI server using PreloadedSharedStorageConnector "
                "that reuses KV caches generated in the pre-fill phase. All engine/server "
    ...
    parser.add_argument(
        "--manifest",
        required=True,
        help="Path to the manifest.json file generated during prefill.",
    )
    ...
    kv_transfer_config = json.dumps(
        {
            "kv_connector": "PreloadedSharedStorageConnector",
            #"kv_role": "kv_both",
            "kv_role": "kv_consumer",
            "kv_connector_extra_config": {
                "shared_storage_path": args.kv_cache_dir,
    ...
    # Forward every argument the user provided *except* --kv-cache-dir and --manifest.
    ...
    # Launch.
    # ------------------------------------------------------------------
    print("[PreloadedDisaggDecodeServer] Starting decode server:\n  " + " ".join(cmd))
    proc = subprocess.Popen(cmd)
    try:
        proc.wait()
    except KeyboardInterrupt:
        print("[PreloadedDisaggDecodeServer] Shutting down server…")
        proc.terminate()
        proc.wait()
